...
void Transform(grostl_state_t *ctx, 
	       const uint8_t *input, 
	       int msglen) {
...
void OutputTransformation(grostl_state_t *ctx) {
  int j;
...
/* initialise context */
int grostl_init(grostl_state_t* ctx,
		int hashbitlen) {
...
int grostl_update(grostl_state_t* ctx,
...
int grostl_final(grostl_state_t* ctx,
		 BitSequence* output) {
...
/* hash bit sequence */
int grostl_hash(int hashbitlen,
		const BitSequence* data, 
		DataLength databitlen,
		BitSequence* hashval) {
  int ret;
  grostl_state_t context;

  /* initialise */
  if ((ret = grostl_init(&context, hashbitlen)) != SUCCESS)
    return ret;

  /* process message */
  if ((ret = grostl_update(&context, data, databitlen)) != SUCCESS)
    return ret;

  /* finalise */
  ret = grostl_final(&context, hashval);
